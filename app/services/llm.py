...
_SUMMARY_PROMPT = """На основе этого отчёта о диагностике бизнеса сформируй краткую сводку — ровно два предложения.
...
Только два предложения, без заголовков, без списков, без вступлений.
...
async def generate_summary(report_text: str) -> str | None:
    """Генерирует двухпредложенную сводку по готовому отчёту. При ошибке возвращает None."""
    t0 = time.perf_counter()
    try:
        token = await _get_token()
        prompt = _SUMMARY_PROMPT.format(report=report_text[:3000])
        payload = {
            "model": "GigaChat",
        ...
        GIGACHAT_REQUESTS.labels(operation="generate_summary", status="success").inc()
        logger.info("GigaChat summary: %s", result)
        return result or None
    except Exception as e:
        GIGACHAT_REQUESTS.labels(operation="generate_summary", status="error").inc()
        logger.warning("Генерация сводки не удалась: %s", e)
        return None
    finally:
        GIGACHAT_DURATION.labels(operation="generate_summary").observe(time.perf_counter() - t0)
